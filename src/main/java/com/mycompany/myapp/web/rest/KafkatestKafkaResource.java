...
@RestController
@RequestMapping("/api/kafkatest-kafka")
public class KafkatestKafkaResource {

    private final Logger log = LoggerFactory.getLogger(KafkatestKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public KafkatestKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
